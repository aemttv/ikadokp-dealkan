<x-admin-layout title="Admin Property">
    {{-- Title --}}
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold">Properti Sekunder Dealkan, Nilai Tambah yang Menjanjikan</h1>
            <p class="text-gray-600">Bantu properti sekunder Anda meraih potensi maksimal.</p>
        </div>
    ...
    <div class="w-full flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
        <form method="GET" action="{{route('propertySecondary.search')}}" class="">
            <div class="flex">
        ...
        <a href="{{ route('addSecondary.view') }}" class="bg-gradient-to-r from-orange-400 to-orange-600 text-white px-4 py-2.5 rounded hover:to-orange-500 text-center">
            Tambah Properti
        </a>
    ...
    <div class="hidden sm:block">
        ...
        @if (!$secondary == [])
            <nav class="text-gray-600 justify-center items-end text-right">
                ...
                <span> &gt; </span>
            <a href="/property-secondary" class="hover:text-gray-800">Properti</a>
            <span> &gt; </span>
            <a href="/property-secondary" class="hover:text-gray-800">Daftar Sekunder</a>
            </nav>
            <p class="text-gray-600 justify-center items-end text-right">
                Menampilkan {{ $secondary->firstItem() }}-{{ $secondary->lastItem() }} dari total
                {{ $secondary->total() }} properti
            </p>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm text-center mt-4">
            <thead class="ltr:text-left rtl:text-right">
                <tr>
                    <th class="whitespace-nowrap px-4 py-2 font-bold text-gray-900">No ID</th>
                    ...
                    <th class="whitespace-nowrap px-4 py-2 font-bold text-gray-900">Sertifikat</th>
                    <th class="whitespace-nowrap px-4 py-2 font-bold text-gray-900">Kondisi</th>
                    <th class="whitespace-nowrap px-4 py-2 font-bold text-gray-900">Gambar</th>
                    <th class="whitespace-nowrap px-4 py-2 font-bold text-gray-900">Interaksi</th>
                </tr>
            </thead>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                @foreach ($secondary as $data)
                    <tr class="odd:bg-gray-100">
                        <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{{ $data->listingID }}</td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-left">{{mb_strimwidth($data->title, 0, 20, '...') }}</td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-left">{{ mb_strimwidth($data->alamat, 0, 20, '...') }}</td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 ">
                            {{ $data->transaksiID == 1 ? 'Dijual' : 'Disewa' }}</td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 ">{{ $listingType[$data->listingType] }} </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700 text-left">{{ $location[$data->lokasiID] }} -
                        ...
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                            {{$sertiType[$data->sertifikatType]}}
                        </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                            {{ $data->kondisiPerabotanID == 1 ? 'Furnished' : ($data->kondisiPerabotanID == 2 ? 'Semi-Furnished' : 'Unfurnished') }}
                        </td>
                        <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        ...
                        <td class="whitespace-nowrap px-4 py-2 flex space-x-2 justify-center items-center">
                            ...
                            <a href="{{route('property.detail', $data->listingID)}}"
                                class="inline-block rounded bg-cyan-600 px-4 py-2 text-xs font-medium text-white hover:bg-cyan-500">
                                Detail
                            </a>
                            <!-- Button Edit -->
                            <a href="{{route('updateSecondary.view', $data->listingID)}}"
                                class="inline-block rounded bg-blue-500 px-4 py-2 text-xs font-medium text-white hover:bg-blue-600">
                                Ubah
                            </a>

                            <!-- Button Delete -->
                            <form action="{{ route('deleteListSecondary', $data->listingID) }}" method="POST" class="inline"
                                onsubmit="return confirm('Are you sure you want to delete this secondary property? This action cannot be undone.');">
                                @csrf
                                {{-- @method('DELETE') --}}
                                <button type="submit"
